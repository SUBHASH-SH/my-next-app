// app/page.tsx
import { getAdmitPosts } from '@/lib/post';
...
export default async function AllJobs() {
  const jobs = await getAdmitCard();

  return (
    <main className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-2xl font-bold text-black mb-6">Admit Cards</h1>
      <ul className="space-y-4">
        {jobs.map((job) => (
          <li key={job.id} className="border border-gray-200 rounded-lg hover:shadow-sm transition-shadow">
            <Link href={`/admit-card/${job.id}`} className="block hover:bg-gray-50">
              <div className="p-4">
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-gray-500">
                      Post Date: {new Date(job.startDate).toLocaleDateString()}
                    </span>
                  </div>
                </div>
              </div>
